...
- `_PROVIDER_PROFILE_GROUP` — plugins that call
    `register_provider_profile(...)` to declare provider- or model-keyed
    `ProviderProfile` entries.
- `_HARNESS_PROFILE_GROUP` — plugins that call
    `register_harness_profile(...)` to declare provider- or model-keyed
    `HarnessProfile` entries.

  (Both group names are string constants defined below; they are part of the
  published plugin contract and must not be renamed casually.)
...
from atlas_engine.kernel.profiles.harness import (
    _anthropic_haiku_4_5,
)
from atlas_engine.kernel.profiles.harness.harness_profiles import _HARNESS_PROFILES
...
entry-point phases complete — so both the kernel's own defaults and any
...
# ★ 接 LLM 网关（Higress）之后的一条约束：
#
#   我们对网关说的是 Anthropic 协议，于是无论背后是什么模型，
#   LangChain 看到的 provider 恒为 "anthropic"：
#       claude-opus-5 → identifier=claude-opus-5  provider=anthropic
#       qwen-max      → identifier=qwen-max       provider=anthropic   ← 背后是 qwen
#
#   所以 **provider 级注册（键为裸 "anthropic"）已经失效** —— 它会把
#   Anthropic 的假设套到任何过网关的模型上。只用精确的
#   "anthropic:<model>" 键，那个模型名是我们自己传的，仍然准。
...
def _ensure_builtin_profiles_loaded() -> None:
    ...
    1. Call the built-in provider `register` functions directly.
        Any exception propagates — a broken built-in is a kernel
        bug and must surface loudly, not degrade silently.
    2. Iterate `importlib.metadata` entry points in the
        `_PROVIDER_PROFILE_GROUP` and `_HARNESS_PROFILE_GROUP` groups.
        Third-party failures are logged at `WARNING` and skipped so one
        misbehaving distribution cannot prevent `kernel.profiles`
        from importing.
    ...
    `get_*_profile` entry points; importing `kernel.profiles` itself
